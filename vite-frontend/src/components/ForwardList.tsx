...
export default function ForwardList({ userId }: ForwardListProps) {
    ...
    const loadData = async (lod = true) => {
        ...
        try {
            ...
            if (forwardsRes.code === 0) {
                let forwardsData = forwardsRes.data?.map((forward: any) => ({
                    ...forward,
                    serviceRunning: forward.status === 1
                })) || [];
                // Filter by userId if provided (Admin view)
                if (userId) {
                    forwardsData = forwardsData.filter((f: Forward) => f.userId === userId);
                }
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                try {
                    ...
                    const response = await createForward({
                        ...
                        strategy: 'fifo',
                        userId: userId // Inject userId for Admin import
                    });
