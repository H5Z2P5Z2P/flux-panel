...
export default function TunnelPage() {
  ...
  const [form, setForm] = useState<TunnelForm>({
    ...
    type: 1,
    nodes: [
      { nodeId: null, protocol: 'tls', tcpListenAddr: '[::]', udpListenAddr: '[::]', interfaceName: '' }
    ],
    flow: 1,
    trafficRatio: 1.0,
    status: 1
  ...
  const validateForm = (): boolean => {
    ...
    if (form.nodes.length === 0 || !form.nodes[0].nodeId) {
      newErrors.inNodeId = '请选择入口节点';
    }

    if (form.nodes[0] && !form.nodes[0].tcpListenAddr.trim()) {
      newErrors.tcpListenAddr = '请输入TCP监听地址';
    }

    if (form.nodes[0] && !form.nodes[0].udpListenAddr.trim()) {
    ...
    if (form.type === 2) {
      if (form.nodes.length < 2) {
        newErrors.outNodeId = '隧道转发模式下，至少需要一个出口节点';
      } else {
        const exitNode = form.nodes[form.nodes.length - 1];
        if (!exitNode.nodeId) {
          newErrors.outNodeId = '请选择出口节点';
        }

        // 检查节点重复
        const nodeIds = form.nodes.map(n => n.nodeId).filter(id => id !== null);
        if (new Set(nodeIds).size !== nodeIds.length) {
          newErrors.nodes = '链路中不能包含重复的物理节点';
        }
      }
    }
  ...
  const handleEdit = (tunnel: Tunnel) => {
    ...
    setForm({
      ...
      name: tunnel.name,
      type: tunnel.type,
      nodes: tunnel.nodes.map(n => ({
        nodeId: n.nodeId,
        protocol: n.protocol || 'tls',
  ...
  const handleTypeChange = (type: number) => {
    setForm(prev => {
      ...
      if (type === 1) {
        // 端口转发，只保留第一个节点
        newNodes = [newNodes[0]];
      } else if (type === 2 && newNodes.length < 2) {
        // 隧道转发，至少加一个出口
        newNodes.push({ nodeId: null, protocol: 'tls', tcpListenAddr: '', udpListenAddr: '', interfaceName: '' });
      }
      return {
        ...prev,
        type,
  ...
  const addNode = () => {
    setForm(prev => ({
      ...prev,
      nodes: [...prev.nodes, { nodeId: null, protocol: 'tls', tcpListenAddr: '', udpListenAddr: '', interfaceName: '' }]
    }));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        status: form.status,
        nodes: form.nodes.map((n, idx) => ({
          nodeId: n.nodeId,
          protocol: idx === 0 ? "" : n.protocol,
          tcpListenAddr: idx === 0 ? n.tcpListenAddr : "",
          udpListenAddr: idx === 0 ? n.udpListenAddr : "",
          interfaceName: idx === 0 ? n.interfaceName : ""
        }))
      };
                ...
                <CardBody className="pt-0 pb-3">
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1.5 relative">
                      {tunnel.nodes && tunnel.nodes.length > 0 && (
                        <>
                          <div className="p-2 bg-default-50 dark:bg-default-100/50 rounded border border-default-200 dark:border-default-300 relative z-10">
                            <div className="flex items-center justify-between mb-1">
                              <span className="text-[10px] font-bold text-success uppercase tracking-wider">入口</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <code className="text-xs font-mono text-foreground truncate">
                                {getNodeName(tunnel.nodes[0].nodeId)}
                              </code>
                            </div>
                          </div>

                          <div className="flex flex-col items-center py-1 gap-1">
                            {tunnel.nodes.length > 2 && (
                          ...
                          <div className="p-2 bg-default-50 dark:bg-default-100/50 rounded border border-default-200 dark:border-default-300 relative z-10">
                            ...
                            <code className="text-xs font-mono text-foreground truncate block">
                              {tunnel.type === 1 ? getNodeName(tunnel.nodes[0].nodeId) : getNodeName(tunnel.nodes[tunnel.nodes.length - 1].nodeId)}
                            </code>
                          </div>
                        </>
                      )}
                      ...
                      <div key={index} className="relative">
                        ...
                        {index > 0 && (
                          <div className="flex justify-center py-2 relative z-0">
                            <div className="absolute top-0 bottom-0 left-1/2 w-0.5 bg-default-200 -z-10" />
                            <div className="bg-background px-2">
                              <svg className="w-4 h-4 text-default-400 animate-bounce" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 14l-7 7m0 0l-7-7m7 7V3" />
                              </svg>
                            </div>
                          </div>
                        ...
                        <Card className="border border-divider shadow-none bg-default-50/50 overflow-visible relative z-10 transition-all hover:border-primary/50">
                          <CardHeader className="flex justify-between items-center py-2 px-4 bg-default-100/30">
                            <div className="flex items-center gap-3">
                              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold text-white shadow-sm ${index === 0 ? 'bg-success' : (index === form.nodes.length - 1 ? 'bg-secondary' : 'bg-warning')
                                }`}>
                                {index + 1}
                              </div>
                              <Chip
                                size="sm"
                                color={index === 0 ? "success" : (index === form.nodes.length - 1 ? "secondary" : "warning")}
                                variant="flat"
                                className="font-medium"
                              >
                                {index === 0 ? "入口节点 (Entry)" : (index === form.nodes.length - 1 ? "出口节点 (Exit)" : `中转节点 ${index} (Transit)`)}
                              </Chip>
                            </div>
                            {form.type === 2 && !isEdit && index > 1 && index < form.nodes.length - 1 && (
                              <Button isIconOnly size="sm" color="danger" variant="light" onPress={() => removeNode(index)}>
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          <CardBody className="p-4 space-y-4">
                            <Select
                              label="选择服务器"
                              placeholder="请选择物理节点"
                              selectedKeys={node.nodeId ? [node.nodeId.toString()] : []}
                              onSelectionChange={(keys) => {
                                const selectedKey = Array.from(keys)[0] as string;
                                if (selectedKey) {
                                  updateNodeField(index, 'nodeId', parseInt(selectedKey));
                                }
                              }}
                              isInvalid={index === 0 ? !!errors.inNodeId : (index === form.nodes.length - 1 ? !!errors.outNodeId : false)}
                              errorMessage={index === 0 ? errors.inNodeId : (index === form.nodes.length - 1 ? errors.outNodeId : '')}
                              variant="bordered"
                              isDisabled={isEdit}
                            >
                              {nodes.map((n) => (
                                <SelectItem
                                  key={n.id}
                                  textValue={`${n.name} (${n.serverIp || '未知IP'})`}
                                >
                                  <div className="flex items-center justify-between">
                                    <div className="flex flex-col">
                                      <span className="text-small font-medium">{n.name}</span>
                                      <span className="text-tiny text-default-400 font-mono tracking-tighter">{n.serverIp}</span>
                                    </div>
                                    <Chip
                                      color={n.status === 1 ? 'success' : 'danger'}
                                      variant="flat"
                                      size="sm"
                                    >
                                      {n.status === 1 ? '在线' : '离线'}
                            ...
                            {index === 0 ? (
                              <>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                  <Input
                                    label="TCP 监听地址"
                                    placeholder="[::] 或 0.0.0.0"
                                    value={node.tcpListenAddr}
                                    onChange={(e) => updateNodeField(index, 'tcpListenAddr', e.target.value)}
                                    isInvalid={!!errors.tcpListenAddr}
                                    errorMessage={errors.tcpListenAddr}
                                    variant="bordered"
                                    startContent={<div className="bg-default-100 px-1 rounded text-[10px] font-bold mr-1">TCP</div>}
                                  />
                                  <Input
                                    label="UDP 监听地址"
                                    placeholder="[::] 或 0.0.0.0"
                                    value={node.udpListenAddr}
                                    onChange={(e) => updateNodeField(index, 'udpListenAddr', e.target.value)}
                                    isInvalid={!!errors.udpListenAddr}
                                    errorMessage={errors.udpListenAddr}
                                    variant="bordered"
                                    startContent={<div className="bg-default-100 px-1 rounded text-[10px] font-bold mr-1">UDP</div>}
                                  />
                                </div>
                                {form.type === 2 && (
                                  <Input
                                    ...
                                    onChange={(e) => updateNodeField(index, 'interfaceName', e.target.value)}
                                    variant="bordered"
                                    description="用于指定多IP服务器的出口流量来源"
                                  />
                                )}
                              </>
                            ) : (
                              <Select
                                label="传输协议"
                                placeholder="选择上跳至本跳的加密方式"
                                selectedKeys={[node.protocol]}
                                onSelectionChange={(keys) => {
                                  const selectedKey = Array.from(keys)[0] as string;
                                  if (selectedKey) {
                                    updateNodeField(index, 'protocol', selectedKey);
                                  }
                                }}
                                variant="bordered"
                              >
                                <SelectItem key="tls">TLS (加密)</SelectItem>
                                <SelectItem key="wss">WSS (WebSocket TLS)</SelectItem>
                                <SelectItem key="tcp">TCP (明文直连)</SelectItem>
                                <SelectItem key="mtls">MTLS (通道级 TLS)</SelectItem>
                                <SelectItem key="mwss">MWSS (通道级 WSS)</SelectItem>
                                <SelectItem key="mtcp">MTCP (通道级 TCP)</SelectItem>
                              </Select>
                            )}
                          </CardBody>
                        </Card>
                  ...
                  <Alert
                    ...
                    description={
                      <div className="text-xs space-y-1 mt-1">
                        <p>1. <b>入站地址</b>：通常填写 <code className="bg-primary-50 px-1 rounded">[::]</code> (全协议栈) 或 <code className="bg-primary-50 px-1 rounded">0.0.0.0</code>。</p>
                        <p>2. <b>链路顺序</b>：流量将按 1 → 2 → ... → N 的顺序依次透传。</p>
                        <p>3. <b>协议选择</b>：跨公网建议使用 <code className="bg-primary-50 px-1 rounded">TLS</code> 相关协议，内网互联可用 <code className="bg-primary-50 px-1 rounded">TCP</code>。</p>
                      </div>
                    }
                  />
                </div>
